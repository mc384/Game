...
bg = pygame.image.load('dungeon10.png')
...
boom = pygame.image.load('smoke_attack.png')
...
# music = pygame.mixer.music.load('soundtrack')
...
run = True
hero = player(300, 410, 64, 64, 100, 2)
...
while run:
    ...
    for i in enemies:
        if abs(hero.x - i.x) < 10 and abs(hero.y - i.y) < 10:
    ...
    if keys[pygame.K_SPACE]:
        if abs(hero.y - goblin.y) < 10 or abs(hero.x - goblin.x) < 10:
            goblin.hit()


    if keys[pygame.K_UP] and hero.y - hero.vel > 100:
        hero.y -= vel
        hero.up = True
        hero.down = False
        hero.standing = False

    if keys[pygame.K_DOWN] and hero.y + hero.vel + hero.height < 980:
        hero.y += vel
        hero.up = False
        hero.down = True
        hero.standing = False

    if keys[pygame.K_LEFT] and hero.x - hero.vel > 200: 
        hero.x -= vel
        hero.left = True
        hero.right = False
        hero.up = False
    ...
    elif keys[pygame.K_RIGHT] and hero.x + vel + length < 1700:  
        hero.x += vel
        hero.left = False
        hero.right = True
        hero.up = False
